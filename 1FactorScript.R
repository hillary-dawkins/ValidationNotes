#The 1-factor model (can follow the 1-factor notebook for more details)
...
student_data <- read.csv('path_to_your_data_here.csv')
#student_data <- student_data[,c(6,7,9,15,16,17,19,21,23,24)] 
#student_data <- student_data[,c(6,7,15,16,17,19,21,23,24)] #Q8 dropped for KMO test
#student_data <- student_data[,c(6,7,15,16,17,19,21,24)] #Q22 dropped for 1 factor model (EFA factor loading)
student_data <- student_data[,c(7,15,16,17,19,21,24)] #Q5 dropped for 1 factor model (CFA) test stats
#student_data <- student_data[,c(7,15,16,17,19,21)] #Q23 dropped for 1 factor model (CFA) test stats
...
fa_1 <- fa(r = PCM, nfactors = 1, fm = "uls")
...
model_1 <- 'F1_A =~ Q3_18 + Q3_6 + Q3_14 + Q3_15 + Q3_16 + Q3_20 + Q3_23'
#model_1 <- 'F1_A =~ Q3_18 + Q3_6 + Q3_14 + Q3_15 + Q3_16 + Q3_20'
...
fit_1 <- cfa(model_1, data = student_data, ordered = c("Q3_6", "Q3_14", "Q3_15", "Q3_16", "Q3_18", "Q3_20", "Q3_23"))
#fit_1 <- cfa(model_1, data = student_data, ordered = c("Q3_6", "Q3_14", "Q3_15", "Q3_16", "Q3_18", "Q3_20"))

#look at some output

#summary(fit_1, fit.measures=TRUE)
sanity_1 <- standardizedSolution(fit_1) #compare factor loadings and uniquenesses to EFA soln (double check output is sensible)
...
####### PART 4: RELIABILITY ##########
# check ordinal alpha for the item measure, and each factor (here we only have 1)
